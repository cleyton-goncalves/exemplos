...
check_last_login() {
  ...
  logins=$(aws cloudtrail lookup-events --profile $PROFILE --region $REGION --lookup-attributes AttributeKey=Username,AttributeValue=$user --query "Events[?EventName=='Authenticate' || EventName=='Federate' || EventName=='UserAuthentication' || EventName=='CredentialVerification' || EventName=='CredentialChallenge'].EventTime" --output table)
